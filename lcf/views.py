...
from .forms import ScenarioForm, AuctionYearTechnologyForm, ScenarioNameForm
from .models import Scenario, AuctionYear, AuctionYearTechnology

# Create your views here.
def scenario_new(request):
    AuctionYearTechnologyFormSet = formset_factory(AuctionYearTechnologyForm, max_num=2)
    ...
    if request.method == "POST":
        ...
        if formset.is_valid() and scenario_name_form.is_valid():
            ...
            for form in formset:
                ...
                AuctionYearTechnology.objects.create(year=auctionyear, technology_name=technology_name.technology_name, strike_price = strike_price, min_levelised_cost=min_levelised_cost)

        return redirect('scenario_detail', pk=scenario.pk)
    else:
        scenario_name_form = ScenarioNameForm()
        formset = AuctionYearTechnologyFormSet(initial= [{'year':AuctionYear.objects.get(pk=1),'technology_name':AuctionYearTechnology.objects.get(pk=1)},
                                                            {'year':AuctionYear.objects.get(pk=2),'technology_name':AuctionYearTechnology.objects.get(pk=2)}])
